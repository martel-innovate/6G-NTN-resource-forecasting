...
@task
def fetch_prometheus_data(metric, start_time_str=None, end_time_str=None):
    global logger
...
@flow
def prometheus_to_postgres(metric: str = "container_cpu_usage_seconds_total", start_time_str: str = "1970-00-00T00:00:00+00:00", end_time_str: str = "1970-00-00T00:00:00+00:00"):
    """Collects Prometheus metric (e.g. CPU or memory) and stores it in DB."""
    global logger
    logger = get_run_logger()   # initialize once per flow run
    raw_data = fetch_prometheus_data(metric, start_time_str, end_time_str)
